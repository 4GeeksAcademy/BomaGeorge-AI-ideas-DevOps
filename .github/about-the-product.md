...
1. Always review this document before implementing, refactoring, or suggesting new features. Use it to align development choices with the product's intent, tone, and audience.
...
Use the following structure to describe the product. Each section contains reserved comment blocks (`<!-- -->`) that signal where to write or edit content.
...
<!-- Define the minimal feature set required for a viable launch. This should correspond to the MVP features in implementation-plan.md. -->
...
<!-- Define key design guidelines and user experience philosophies. Examples: simplicity, clarity, accessibility, modern UI, responsive layout, etc. -->
...
Data/Auth: Supabase (PostgreSQL for deployments, environments, alerts, SLI/SLO targets; Supabase Auth for sessions).
Real-time: Supabase Realtime channels and/or WebSockets/SSE for status updates.
...
Hosting: Vercel (web/API) + Supabase project (DB/Auth/Realtime); lightweight worker on Vercel cron or a microservice (Render/Fly) if needed.

---

## Product Structure Overview

<!-- Optionally, summarize major app sections or components at a high level (e.g., Dashboard, Profile, Reports). This helps the AI understand the architecture contextually. -->
...
- Environment Status: current version and health per environment.
...
- 2025-11-19 - Defined dual MVPs: Deployment & Rollback Manager and Infrastructure Health Dashboard; selected Next.js + Supabase (DB/Auth/Realtime) + Vercel hosting; metrics/log ingestion via Prometheus/OTel and OpenSearch/Elastic.
